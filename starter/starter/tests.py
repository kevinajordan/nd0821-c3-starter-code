...
def test_train_model(synthetic_census_data):
    ...
    Ensures that the function returns an XGBClassifier instance with the
...
def test_compute_model_metrics(synthetic_census_data):
    ...
    Verifies that the function returns precision, recall, and fbeta scores
    between 0 and 1.
    """
    X, y, _, _ = process_data(
        synthetic_census_data,
...
def test_inference(synthetic_census_data):
    ...
    Ensures that the function returns predictions as a numpy array with the
    correct shape.
    """
    X, y, _, _ = process_data(
        synthetic_census_data,
...
def test_process_data(synthetic_census_data):
    ...
    Checks the function's behavior in training mode, inference mode, and
    without a label. Ensures correct output types and shapes for each scenario.
    """
    categorical_features = [
        "workclass", "education", "marital-status", "occupation",
        "relationship", "race", "sex", "native-country"
